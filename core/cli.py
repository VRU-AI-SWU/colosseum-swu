...
def cmd_serve(args) -> int:
    ...
    if args.real_seeds:
        ...
        if secrets_root() is None:
            ...
            return 1
        # ⚠️ **ข้อความนี้พูดเรื่อง seed อย่างเดียว ไม่พูดเรื่องที่เก็บข้อมูล**
        # เดิมเขียนว่า "ข้อมูลอยู่ในหน่วยความจำ" ติดมาด้วยเสมอ ซึ่งขัดกับบรรทัด
        # `ฐานข้อมูล : ...` ที่เพิ่งพิมพ์ไปสองบรรทัดก่อนหน้า · บนเครื่องจริงมันอ่าน
        # เหมือนเตือนว่าข้อมูลจะหาย ทั้งที่เก็บลงดิสก์อยู่ · เรื่องที่เก็บข้อมูล
        # มีบรรทัดของตัวเองอยู่แล้ว (`--ephemeral`)
        print("seed       : ✅ ชุดจริงจาก ARENA_SECRETS — คะแนนเทียบ leaderboard ได้")
    else:
        print("seed       : ⚠️ ชุดสำรองที่ไม่ใช่ของจริง — คะแนนที่ได้เทียบ leaderboard ไม่ได้")
    uvicorn.run(
        create_app(
            arena,
