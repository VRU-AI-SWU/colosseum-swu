...
### 4.6 ~~ขนาดทีมเป็นค่าคงที่ในโค้ด~~ ✅ ปิดแล้ว

`MAX_TEAM_SIZE = 6` เคยอยู่ใน `core/service.py` การจะเปลี่ยนขนาดทีมจึงต้องแก้โค้ด
แล้ว deploy ใหม่ ซึ่งไม่ใช่สิ่งที่ผู้สอนควรต้องทำกลางเทอม

ตอนนี้มี entity `Course` (schema v3) ที่เก็บ `max_team_size` และผู้สอนแก้ได้จาก
แถบบนหน้าเว็บ · `MAX_TEAM_SIZE` ยังอยู่แต่กลายเป็นแค่**ค่าเริ่มต้นของวิชาใหม่**

**เรื่องที่ยากคือสิทธิ์ ไม่ใช่การเก็บค่า** — โทเคนใช้ร่วมกันทั้งทีม

| ตรวจแบบ | ผลที่ตามมา |
|---|---|
| "มีผู้สอนอยู่ในทีมก็พอ" | นิสิตที่อยู่ทีมเดียวกับผู้สอนถือโทเคนที่เปลี่ยนกติกาทั้งวิชาได้ |
| **"ทุกคนในทีมต้องเป็นผู้สอน"** ← ที่ใช้จริง | ผู้สอนต้องมีทีมเดี่ยวของตัวเอง ซึ่งเป็นสิ่งที่เกิดขึ้นอยู่แล้ว |

รายชื่อผู้สอนมาจาก `ARENA_STAFF_EMAILS` ใน `/etc/arena.env` **ไม่ใช่จากฐานข้อมูล** —
เหมือน sudoers ถ้าแก้ผ่านหน้าเว็บได้ คนที่ยึดสิทธิ์ได้ครั้งเดียวจะแต่งตั้งตัวเองถาวร
· ว่างไว้ = ไม่มีใครเป็นผู้สอน ซึ่งเป็นค่าเริ่มต้นที่ปลอดภัยโดยปริยาย

การซ่อนแผงบนหน้าเว็บ **ไม่ใช่ความปลอดภัย** — endpoint ตรวจสิทธิ์ซ้ำเสมอ
เพราะยิงตรงด้วย `curl` ได้ และโทเคนอยู่ในมือนิสิตทุกคนอยู่แล้ว

การลดขนาดทีมต่ำกว่าทีมที่มีอยู่ถูกปฏิเสธพร้อมบอกชื่อทีมที่เป็นปัญหา — ยอมแล้วปล่อยให้
มีทีมที่ผิดกติกาของตัวเองอยู่ จะกลายเป็นข้อโต้แย้งตอนตัดเกรด

### 4.7 ~~ระบบรองรับวิชาเดียว~~ ✅ ปิดแล้ว (schema v4)

`_default_course_id()` เดา course จาก competition อันแรก ซึ่งใช้ได้ตอนมีวิชาเดียว
เพราะเดาถูกเสมอ · พอมีวิชาที่สอง (Machine Learning) การเดาจะพาคนไปอยู่ผิดวิชา
โดยไม่มีใครรู้

**paradigm ติดที่ competition ไม่ใช่ครอบวิชา**

```
Course ──< Competition >── Paradigm
```

แบบ "paradigm ครอบวิชา" ที่ดูตรงไปตรงมากว่า พังกับวิชาที่มีอยู่จริง — CP463 (AI)
มีทั้งโจทย์ RL และ Tool-use Agent ที่วางแผนไว้ ส่วนวิชา ML มีทั้ง supervised
และ unsupervised · **วิชาหนึ่งคร่อมหลาย paradigm โดยธรรมชาติ**

paradigm ยังแยกจาก `task_type` ด้วย — อันแรกบอกว่านิสิต*เรียนอะไร* อันหลังบอกว่า
ระบบ*ให้คะแนนยังไง* · supervised กับ unsupervised ใช้ runner ตัวเดียวกัน (prediction)
แต่เป็นคนละ paradigm ในสายตานิสิต

**โทเคนย้ายจากทีมมาที่คน** — ไม่ได้เลือกเอง แต่ถูกบังคับโดยโครงสร้าง

| | โทเคนของทีม (เดิม) | โทเคนของคน |
|---|---|---|
| หลายวิชา | หลายโทเคน ต้องสลับ | อันเดียวใช้ได้ทุกวิชา |
| ล็อกอินครั้งแรก | ยังไม่มีทีม → ไม่มีอะไรส่งกลับ | มีโทเคนทันที ใช้เข้าวิชาต่อได้ |
| โทเคนหลุด | ทั้งทีมต้องเปลี่ยน | เฉพาะคนนั้น |
| สิทธิ์ผู้สอน | ต้องมีกฎ "ทั้งทีมต้องเป็นผู้สอน" | ถามจากคนตรงๆ — **ลบกฎนั้นทิ้งได้** |
| `actor_id` ใน audit | เดาจากสมาชิกคนแรก | รู้จริงว่าใครกด |

`arena submit <slug>` บอก competition อยู่แล้ว เซิร์ฟเวอร์จึงหาทีมจาก
"คนนี้ + วิชาของ competition นั้น" (`Arena.team_for`)

**การเข้าวิชาใช้รหัสที่ผู้สอนแจกในคาบ** — บัญชี `@g.swu.ac.th` มีทุกคนใน
มหาวิทยาลัย ไม่ใช่แค่คนในวิชานี้ · คนที่ล็อกอินได้แต่ไม่มีรหัสจะไม่มีทีมและ
ไม่โผล่ที่ไหนเลย ซึ่งถูกต้อง

⚠️ **ข้อจำกัดที่รู้ตัว** — `ARENA_STAFF_EMAILS` เป็นรายชื่อของทั้ง*เครื่อง*
ไม่ใช่รายวิชา · วันนี้ผู้สอนคนเดียวสอนทุกวิชาบนเครื่องนี้จึงยังไม่เป็นปัญหา
ถ้าวันหนึ่งมีผู้สอนหลายคนแบ่งกันคนละวิชา ต้องมีรายชื่อรายวิชา
(มีเทสต์ `test_staff_authority_is_deployment_wide_not_per_course` บันทึกไว้)

**สร้างวิชาใหม่ด้วย [`tools/setup_course.py`](../tools/setup_course.py)** ไม่ใช่จาก
หน้าเว็บ — การสร้างวิชาเกิดปีละไม่กี่ครั้งและเป็นจังหวะที่ตัดสินใจเรื่องที่แก้ยาก
(`id` ของวิชาไปโผล่ใน `Team.course_id` ของทุกทีมและใน audit trail เปลี่ยนทีหลัง
แปลว่าต้องไล่แก้ทุกแถว) · ฟอร์มบนหน้าเว็บชวนให้กดเร็วโดยไม่คิด

### 4.8 🔻 ต้องทำก่อนเปิดให้นิสิตใช้จริง
