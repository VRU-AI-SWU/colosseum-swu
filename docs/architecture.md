...
เรียงตามความเสี่ยง — ทั้งสองข้อแรกเป็นเรื่องที่**เอกสารสัญญาไว้แต่โค้ดยังไม่ทำ**

### 4.1 `smoke_test()` ไม่ถูกเรียกใช้จริง

[`runners/agent_env/validate.py`](../runners/agent_env/validate.py) มี `smoke_test()` ที่จับ
state รั่วข้าม episode, action ที่ไม่ถูกต้อง และ agent ที่ crash — เขียนครบและมีเทสต์ 6 ข้อ
**แต่ไม่มีใครเรียกมันนอกจากเทสต์** worker เรียก `run_submission()` ตรงๆ

ผลคือ [starter/agent.py](../envs/cp463-vacuum/vacuum/starter/agent.py) ที่บอกนิสิตว่า
"ระบบตรวจข้อนี้ตอนรับ submission และ**ปฏิเสธ**ถ้าไม่ผ่าน" **ยังไม่จริง** — agent ที่ `reset()`
ไม่สะอาดจะถูกรับเข้าไปแล้วได้คะแนนต่ำกว่าที่ควร โดยไม่มีใครบอกว่าเพราะอะไร

### 4.2 `arena serve` ใช้ SubprocessLauncher ไม่ใช่ Docker

`Worker.launcher` มีค่าเริ่มต้นเป็น `SubprocessLauncher` และ CLI ยังไม่มีธงให้เลือก
`DockerLauncher` — ซึ่งเป็นตัวที่มี `--network none`, `--user 10001`, `--read-only`,
`--cap-drop ALL` ทั้งหมด และผ่านเทสต์ 8 ข้อแล้ว

แปลว่า**ถ้าเปิดให้นิสิตส่งงานจริงวันนี้ โค้ดของนิสิตจะรันโดยไม่มี container ห่อ**
ต้องต่อธงเลือก launcher ก่อนเปิดใช้จริง
