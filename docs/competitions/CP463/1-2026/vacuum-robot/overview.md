...
✅ **คะแนนที่ตรึงแล้ว** — วัดบน **public seeds ชุดจริง** (30 seed ต่อ phase · `env_version` 1.0.0)
ชุดเดียวกับที่ใช้ให้คะแนนนิสิตระหว่างเทอม จึงเทียบกับ leaderboard ได้ตรงๆ
ค่าอยู่ที่ [`core/baseline_pins/`](../../../../../core/baseline_pins/) · รันซ้ำด้วย `python tools/pin_baselines.py`

| | Warm-up | Main | Final |
|---|---|---|---|
| 🥉 Bronze | 0.130 | 0.273 | 0.144 |
| 🥈 Silver | 0.784 (ครบ 7/30) | 0.741 (ครบ 2/30) | 0.559 (ครบ 0/30) |
| 🥇 Gold | 1.752 (ครบ 30/30) | 1.547 (ครบ 23/30) | 1.095 (ครบ 10/30) |
| 💎 Diamond | 1.752 (ครบ 30/30) | 1.810 (ครบ 30/30) | 1.725 (ครบ 28/30) |
| เพดานของสูตร | 2.0 | 2.0 | 2.0 |

> ⚠️ **ค่าชุดนี้แทนที่ตัวเลขจากชุด conformance ที่เคยรายงานไว้** — ต่างกันได้ถึง 0.17
> (Gold บน Main: 1.716 → 1.547) เพราะเป็นคนละห้อง หมุดบน leaderboard ต้องมาจาก
> public seeds เท่านั้น ส่วนค่าของชุด conformance มีไว้จับ regression ของ environment
> ([golden_baselines.json](../../../../../envs/cp463-vacuum/vacuum/golden_baselines.json)) ซึ่งเป็นคนละหน้าที่กัน
...
|---|---|---|---|
| Warm-up | +0.654 [+0.468, +0.864] ✅ | +0.967 [+0.757, +1.156] ✅ | +0.000 (เท่ากันตามที่ออกแบบ) |
| Main | +0.467 [+0.382, +0.584] ✅ | +0.807 [+0.629, +0.967] ✅ | +0.262 [+0.127, +0.429] ✅ |
| Final | +0.415 [+0.377, +0.452] ✅ | +0.536 [+0.367, +0.714] ✅ | +0.630 [+0.396, +0.830] ✅ |

**ทุกช่องว่างแยกกันได้ทางสถิติบน public seeds** (ยกเว้น Warm-up Gold→Diamond ที่ต้องเท่ากัน)
จึงใช้เป็นเส้นแบ่งเกรดได้ · ข้อสังเกต: ช่องว่าง Gold → Diamond **กว้างขึ้นเรื่อยๆ ตาม phase**
(+0.00 → +0.26 → +0.63) ซึ่งตรงกับที่ออกแบบไว้ว่าทักษะการจัดการความไม่แน่นอนเป็นตัวตัดสินใน Final
...
| การวางแผนเส้นทางบนแผนที่ และการเลือกเป้าหมายสำรวจ | ❌ | คือก้าว Silver → Gold ซึ่งเป็นช่องว่างที่กว้างที่สุด (+0.70) และเป็นหัวใจของ "สร้างแบบจำลองแล้ววางแผนบนมัน" |
| การสะสมหลักฐานภายใต้ noise | ❌ | คือก้าว Gold → Diamond และตรงกับเนื้อหาคาบ 3–5 พอดี |
