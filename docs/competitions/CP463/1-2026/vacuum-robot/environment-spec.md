...
| `sensor_noise` (Final) | 0.05 | **0.01** | ที่ 0.05 Gold ดูดครบแค่ 2/30 → `completion_bonus` แทบไม่ทำงาน · ที่ 0.01 ได้ 22/30 = 73% ตรงกลางช่วงเป้า 60–90% |
...
⚠️ **ยังตอบไม่ครบ** — การทดลองนี้ต้องมีฝั่ง learned policy มาเทียบด้วย
`examples/calibrate.py --policy module:Class` รออยู่แล้ว แต่ยังไม่มี PPO ที่เทรนเสร็จ
...
✅ **ผล: ผ่านทุก phase หลังปรับ** (Gold ดูดครบ 30/30 · 28/30 · 22/30) แต่มีข้อสังเกตสำคัญ 2 ข้อ
...
| ค่า | สถานะ |
|---|---|
| `sensor_noise` ทั้ง 3 phase | ✅ ตรึงชั่วคราว (0.00 / 0.02 / 0.01) — **รอผลการทดลองที่ 1 ฝั่ง PPO ก่อนตรึงถาวร** |
...
1. **เทรน PPO แล้วปิดการทดลองที่ 1** — ถ้า PPO ยังแพ้ Gold ที่ `sensor_noise: 0.02`
   ต้องกลับไปพิจารณาทางเลือก C+D ของ [template §5](../../../../task-templates/agent-vs-environment-rl.md#5-ปัญหาที่ต้องตัดสินใจ-planning-ชนะ-learning)
   (ยอมรับว่า planner แตะ Gold ได้ แล้ววัดวิธีการที่คะแนนรายงาน 25%)
