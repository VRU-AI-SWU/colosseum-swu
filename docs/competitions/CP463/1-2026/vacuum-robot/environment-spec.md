...
ทั้งสี่ตัวเป็นหมุดหมายบน leaderboard — **สเปคของ Gold มีผลต่อเกรดโดยตรง จึงต้องตรึงให้แน่น**

| | โค้ดอยู่ไหน |
|---|---|
| 🥉 Bronze · 🥈 Silver | `vacuum/baselines/` — แจกไปกับ starter kit อ่านได้เต็ม |
| 🥇 Gold · 💎 Diamond | 🔒 `colosseum-hypogeum/agents/cp463-vacuum/` ([overview §6.1](overview.md#61-ทำไม-gold-กับ-diamond-ถึงไม่แจกโค้ด)) |
...
"จำแผนที่ได้และวางแผนเป็น" — สะสมสิ่งที่เห็นลงแผนที่ แล้ววางแผนบนแผนที่นั้นเพื่อไปหา
ฝุ่นที่รู้ตำแหน่ง หรือไปสำรวจขอบของพื้นที่ที่ยังไม่เคยเห็น replan ใหม่ทุก timestep

**หมายเหตุที่ต้องประกาศพร้อมหมุด** — Gold **เชื่อเซนเซอร์ตรงๆ** เขียนทับแผนที่ด้วยค่าที่เห็น
ล่าสุดโดยไม่กรองอะไรเลย นี่เป็นการตัดสินใจที่ตั้งใจ เพราะทำให้ช่องว่าง Gold → Diamond
วัด "การจัดการความไม่แน่นอน" ล้วนๆ

### 💎 Diamond — `BeliefBFSAgent` 🔒

"รู้ว่าเซนเซอร์เชื่อไม่ได้ แล้วทำอะไรกับมัน" — ตรรกะการวางแผนเหมือน Gold ทุกบรรทัด
ต่างกันจุดเดียวคือมันสะสมหลักฐานจากการสังเกตซ้ำ โดยใช้ค่า `sensor_noise` ที่**ประกาศไว้ใน
config ตรงๆ** แทนที่จะเชื่อค่าล่าสุด

**ไม่จำกัดตระกูลอัลกอริทึม** — ร่างแรกกำหนดว่า Diamond ต้องเป็น learned policy และเขียนว่า
"ถ้า Diamond ที่เทรนแล้วยังชนะ Gold ไม่ได้ แปลว่า config ผิด" ซึ่ง**ยกเลิกแล้ว** เพราะวัดจริง
แล้วพบว่า planner ชนะ learned policy ในโจทย์นี้โดยธรรมชาติ ไม่ใช่เพราะ config ผิด
([overview §12](overview.md#12-ทำไม-planning-ถึงเหมาะกับโจทย์นี้--สิ่งที่นิสิตต้องอธิบายได้))

ผลที่วัดได้: [calibration §1.5](calibration-2026-08.md#15-sensor_noise-ไม่ใช่คันโยกความยาก--มันคือข้อสอบวัดความสามารถ)

> 🔒 **สองระดับนี้ไม่แจกทั้งโค้ดและ pseudocode**
>
> `colosseum-swu` เป็น repo **สาธารณะ** — ร่างก่อนหน้ามี pseudocode ของทั้งสองระดับเต็มๆ
> อยู่ตรงนี้ รวมถึงบรรทัดที่ชี้กับดักที่ยากที่สุด ซึ่งทำให้การเก็บโค้ดไว้ที่อื่นไม่มีความหมาย
>
> สเปคเต็มย้ายไป `colosseum-hypogeum/docs/cp463-1-2026/vacuum/baseline-solutions.md`
> และโค้ดอยู่ที่ `colosseum-hypogeum/agents/cp463-vacuum/` โหลดเข้า `BASELINES` ผ่าน
> `ARENA_SECRETS` เท่านั้น ([overview §6.1](overview.md#61-ทำไม-gold-กับ-diamond-ถึงไม่แจกโค้ด))
>
> เส้นแบ่ง: **แนวคิดเปิด · วิธี implement ปิด** — สิ่งที่เขียนไว้ข้างบนนี้พอให้นิสิตรู้ว่า
> เป้าหมายคืออะไรและใช้ทฤษฎีบทไหน แต่ไม่พอให้ลอกไปเขียน
