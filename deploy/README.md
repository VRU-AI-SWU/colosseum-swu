...
ARENA_COURSE_STAFF_CP462_1_2026=aj@g.swu.ac.th
...
อัปโหลดลงไป ส่วน worker อ่านกลับมาแบ่งตอนให้คะแนน · ถ้าชี้คนละที่ ผู้สอนจะสร้าง
โจทย์ได้ตามปกติแล้วทุก run ล้มด้วย "ไม่พบชุดข้อมูล … ในคลัง"

```bash
install -d -m 700 $ARENA/data/datasets
```

**โหมด 700 ไม่ใช่ 755** — ไฟล์ในนี้คือเฉลยของทั้งวิชา · สิ่งที่กันนิสิตไว้คือ
*ตัวข้อมูล* ไม่ใช่ตัวเลขลับอีกต่อไป การอ่านไฟล์ได้จึงเท่ากับได้เฉลยครบทุกแถว
· ห้าม mount เข้า container ของนิสิตเด็ดขาด ด้วยเหตุผลเดียวกับ `secrets/`

โฟลเดอร์นี้ **ไม่ใช่** ที่เดียวกับ `secrets/` — `secrets/` เป็น clone ของ repo
ส่วนตัวที่มีประวัติการแก้ ส่วนคลังนี้เป็นไฟล์ที่ผู้สอนอัปโหลดผ่านหน้าเว็บ จึงอยู่
ใต้ `data/` ที่มีแผนสำรองข้อมูลอยู่แล้ว
...
⚠️ **สิทธิ์เป็นของทั้งเครื่อง ไม่ใช่รายวิชา** — ตั้งแต่ schema v4 โทเคนเป็นของคน
ไม่ใช่ของทีม สิทธิ์จึงถามจากอีเมลตรงๆ · แต่คนที่อยู่ในรายชื่อนี้แก้ค่าได้**ทุกวิชา**
บนเครื่องนี้ ถ้าวันหนึ่งมีผู้สอนหลายคนแบ่งกันคนละวิชา ต้องเปลี่ยนเป็นรายชื่อรายวิชา
