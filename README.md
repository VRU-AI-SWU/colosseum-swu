...
> ⚠️ **หัวข้อ §10 นี้เป็นเป้าหมายที่ออกแบบไว้ ไม่ใช่สถานะปัจจุบัน** — บางส่วนยังไม่ได้สร้าง
> โดยเฉพาะ runner daemon ที่ต่อ WebSocket, Postgres และ Object Storage
>
> **ระบบที่รันได้จริงวันนี้อยู่ที่ [docs/architecture.md](docs/architecture.md)**
> พร้อม diagram ของ deployment, เส้นทาง submission, ขอบเขตความไว้วางใจ และรายการ
> ช่องว่างที่รู้ตัว · อ่านไฟล์นั้นก่อนถ้าจะแก้โค้ด อ่านหัวข้อนี้ถ้าจะเข้าใจว่าทำไมออกแบบแบบนี้
...
| **โค้ด**ของ baseline ระดับ Gold และ Diamond | 🔒 | ลอกได้ — และสองระดับนี้*คือ*ตัวโจทย์ (ก้าว Silver→Gold คือ "สร้างแบบจำลองแล้ววางแผนบนมัน" · Gold→Diamond คือการจัดการความไม่แน่นอน) แจกไปก็เท่ากับแจกเฉลย · อยู่ที่ `colosseum-hypogeum/agents/` |
...
|---|---|---|
| 1 | ~~ขนาดทีมและจำนวนทีม~~ | ✅ **ตัดสินแล้ว (ส.ค. 2026): ทีมละไม่เกิน 6 คน** · นิสิตจัดทีมกันเอง คนที่หากลุ่มไม่ได้เป็นทีมเดี่ยวได้ทันทีโดยไม่ต้องทำอะไรเพิ่ม · ค่าอยู่ที่ `MAX_TEAM_SIZE` ใน [`core/service.py`](core/service.py) |
...
| 4 | ~~Cloud provider และงบ~~ | ✅ **ตัดสินแล้ว**: Cloudflare (หน้าเว็บ) + เครื่อง GPU ในมหาวิทยาลัย (API + ข้อมูล) เชื่อมด้วย tunnel · ดู [deploy/](deploy/README.md) |
| 5 | ~~บังคับ Google OAuth ของมหาวิทยาลัยหรือรับ Gmail ทั่วไป~~ | ✅ **ตัดสินแล้ว (ส.ค. 2026): บังคับ Google Workspace `@g.swu.ac.th` เท่านั้น** — ไม่รับ Gmail ทั่วไป · ได้การจำกัดโดเมนฟรีด้วยพารามิเตอร์ `hd` และไม่ต้องทำ OTP ทางอีเมลเอง (ไม่ต้องมี SMTP · ไม่ต้องจัดการ deliverability, rate limit, code หมดอายุ) |
...
| `web/` | ✅ หน้า leaderboard ขึ้นแล้วที่ [colosseum.vru-ai.com](https://colosseum.vru-ai.com) ([`web/`](web/)) |
